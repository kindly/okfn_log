...
class Loggy(Bot): 
   ...
   def log(self, line, channel): 
      ...
      if channel not in self.channels:
          return
      try:
          path = os.path.join(self.logdir, channel.strip('#'))
          os.mkdir(path)
          os.chown(path, -1, www_data_gid)
      except OSError, e:
          pass
      logfile = os.path.join(self.logdir, channel.strip('#'), name)
      ...
      try: 
         ...
         f.close()
         os.chown(logfile, -1, www_data_gid)
      except Exception, e: 
...
def main(): 
   ...
   uri = sys.argv[2]
   scheme, _, host, channels = tuple(uri.split('/'))
   
   channels = ['#' + channel for channel in channels.split(',')]
